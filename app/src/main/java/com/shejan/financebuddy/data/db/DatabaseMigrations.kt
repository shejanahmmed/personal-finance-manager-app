...
object DatabaseMigrations {
    ...
    // ─────────────────────────────────────────────────────────
    // v14 → v15 : Add status column to pending_sms_transactions table
    // ─────────────────────────────────────────────────────────
    val MIGRATION_14_15 = object : Migration(14, 15) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `pending_sms_transactions` ADD COLUMN `status` TEXT NOT NULL DEFAULT 'PENDING'")
        }
    }

    // ─────────────────────────────────────────────────────────
    // v15 → v16 : Create investments table
    // ─────────────────────────────────────────────────────────
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS `investments` (
                    ...
                    `expectedReturnRate` REAL NOT NULL DEFAULT 0.0,
